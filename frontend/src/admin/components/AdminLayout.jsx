...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen flex" style={{ background: 'linear-gradient(135deg,#fdf9f6 0%,#f8f2ec 35%,#f3ebe3 65%,#eee4da 100%)' }}>
      ...
      {/* ── Glass Sidebar ── */}
      <aside
        className={`${sidebarOpen ? 'w-64' : 'w-16'} glass-sidebar flex flex-col transition-all duration-300 fixed h-full z-30`}
        style={{ borderRadius: '0 24px 24px 0' }}
      >
        {/* Logo */}
        <div className="h-16 flex items-center px-4 gap-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.5)' }}>
          <div
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-0.5">
          {NAV.map(({ to, icon, label }) => (
            <NavLink
              ...
              to={to}
              className={({ isActive }) =>
                `admin-nav-item ${isActive ? 'active' : ''} ${!sidebarOpen ? 'justify-center px-2' : ''}`
              }
        ...
        <div className="p-3" style={{ borderTop: '1px solid rgba(255,255,255,0.5)' }}>
          {sidebarOpen ? (
            <div className="flex items-center gap-2.5 px-2 py-2 rounded-2xl" style={{ background: 'rgba(255,255,255,0.45)' }}>
              ...
              <button onClick={handleLogout} title="Logout" className="text-gray-400 hover:text-red-500 transition-colors p-1 rounded-lg hover:bg-red-50/50">
            ...
            <button onClick={handleLogout} className="text-gray-400 hover:text-red-500 mx-auto block">
      ...
      {/* ── Main content ── */}
      <div className={`relative z-10 flex-1 flex flex-col ${sidebarOpen ? 'ml-64' : 'ml-16'} transition-all duration-300`}>
        ...
        >
          <button
            ...
            className="p-2 transition-colors text-gray-500 hover:text-primary"
          ...
          <div className="flex items-center gap-3">
            <a
              ...
              className="text-sm text-gray-500 hover:text-primary flex items-center gap-1.5 transition-colors px-3 py-1.5"
            ...
            <div
              className="w-8 h-8 rounded-full flex items-center justify-center text-primary font-bold text-sm"
