...
export default function VendorPricing() {
  ...
  useEffect(() => {
    axios.get(`${API}/vendor/store`, { withCredentials: true })
      .then(({ data }) => {
        const saved = data.data?.makingCharges || [];
        if (saved.length > 0) {
          setCharges(
            DEFAULT_CATEGORIES.map((cat) => {
              const found = saved.find((s) => s.category === cat);
              return found ? { category: cat, type: found.type, value: String(found.value) } : { category: cat, type: 'flat', value: '' };
            })
          );
        }
      })
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.put(`${API}/vendor/store`, { makingCharges: payload }, { withCredentials: true });
      toast.success('Making charges saved!');
    } catch { toast.error('Failed to save charges'); }
      ...
      <div className="p-4 sm:p-6 max-w-2xl space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-4 rounded-xl" style={{ background: 'rgba(201,168,76,0.08)', border: '1px solid rgba(201,168,76,0.2)' }}>
            ...
            <p className="text-xs text-amber-500 mt-1 font-mono">Example: Ring + ₹800 making</p>
          </div>
          <div className="p-4 rounded-xl" style={{ background: 'rgba(90,138,191,0.08)', border: '1px solid rgba(90,138,191,0.2)' }}>
            ...
            <p className="text-xs text-blue-500 mt-1 font-mono">Example: Necklace + 12% on metal cost</p>
          </div>
        </div>
